...
"post",
Route = "blogposts")] HttpRequestData req, [CosmosDBInput
(Connection = "CosmosDbConnectionString")]
CosmosClient client)
    {
        ...
        var savedBlogpost = await SaveBlogpostAsync(blogpost, id, author, database);
...
IsAuthorized(HttpRequestData req)
    {
        if (StaticWebAppApiAuthentication.TryParseHttpHeaderForClientPrincipal
...
(Blogpost blogpost, Database database)
    {
        var tagsContainer = database.GetContainer("StringContainer");
...
Container blogpostContainer)
    {
        ...
        try
        {
            var blogResponse = await blogpostContainer.ReadItemAsync<Blogpost>(id: id,
...
Database database)
    {
        ...
        var container = database.GetContainer("BlogContainer");
        await container.UpsertItemAsync(savedBlogPost, new PartitionKey(author));
